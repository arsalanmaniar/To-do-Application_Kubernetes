...
export default function ErrorDisplay({ error, title = 'An error occurred', onRetry, onDismiss }: ErrorDisplayProps) {
  if (!error) {
    return null;
  }

  // Normalize the error to a consistent format
  const errorMessage = typeof error === 'string' ? error : error.message || 'Unknown error occurred';
  const errorTitle = typeof error === 'string' ? title : (error.name || title);
